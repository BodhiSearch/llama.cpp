...
RUN HIPCXX="$(hipconfig -l)/clang" HIP_PATH="$(hipconfig -R)" \
    cmake -S . -B build -DGGML_HIP=ON -DGGML_HIP_ROCWMMA_FATTN=ON -DAMDGPU_TARGETS=$ROCM_DOCKER_ARCH -DGGML_BACKEND_DL=ON -DGGML_CPU_ALL_VARIANTS=ON -DCMAKE_BUILD_TYPE=Release -DLLAMA_BUILD_TESTS=OFF -DCMAKE_EXE_LINKER_FLAGS="-Wl,-rpath,\$ORIGIN" \
...
RUN PLATFORM_TRIPLE="x86_64-unknown-linux-gnu" && \
    ...
    find /app/lib -name "*.so*" -exec cp -P {} /app/bin/$PLATFORM_TRIPLE/rocm/ \; 2>/dev/null || true
