...
RUN cmake -B build -DGGML_NATIVE=OFF -DGGML_VULKAN=ON -DLLAMA_BUILD_TESTS=OFF -DLLAMA_BUILD_UI=OFF -DLLAMA_CURL=OFF -DLLAMA_OPENSSL=OFF -DGGML_BACKEND_DL=ON -DGGML_CPU_ALL_VARIANTS=ON && \
...
RUN mkdir -p /app/bin/x86_64-unknown-linux-gnu/vulkan && \
    ...
    find /app/lib -name "*.so*" -exec cp -P {} /app/bin/x86_64-unknown-linux-gnu/vulkan/ \; 2>/dev/null || true
