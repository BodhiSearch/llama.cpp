...
RUN ARCH=$(uname -m) && \
    ...
    cp /app/lib/*.so /app/bin/$PLATFORM_TRIPLE/cann/ 2>/dev/null || true
...
LABEL bodhi.ascend.soc="${ASCEND_SOC_TYPE}"
...
RUN ARCH=$(uname -m) && \
    ...
    echo "{\"version\":\"${BUILD_VERSION}\",\"commit\":\"${BUILD_COMMIT}\",\"timestamp\":\"${BUILD_TIMESTAMP}\",\"variant\":\"cann\",\"branch\":\"${BUILD_BRANCH}\",\"cann_base_image\":\"${CANN_BASE_IMAGE}\",\"ascend_soc\":\"${ASCEND_SOC_TYPE}\",\"platform\":\"$PLATFORM_TRIPLE\"}" > /app/version.json && \
